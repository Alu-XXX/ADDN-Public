...
parser.add_argument('--data', type=str, default='/home/cwuu/ADDN_temp/SC-new/tiny-imagenet-200', help='location of the data corpus')
parser.add_argument('--batch_size', type=int, default=54, help='batch size')
...
class midden_block(nn.Module):
    def __init__(self) -> None:
        ...
        self.net=nn.Sequential(
            ...
            # nn.BatchNorm2d(16),
            # nn.ReLU(),
            nn.Conv2d(256,200,kernel_size=1),
...
for param in midden_net.parameters():
    param.requires_grad = False
ops=nn.ModuleList()
ops.append(nn.Sequential(
  nn.ReLU(),
  nn.Conv2d(64,512,kernel_size=6,padding=1,stride=4),
  nn.BatchNorm2d(512),
...
ops.append(nn.Sequential(
  nn.ReLU(),
  nn.Conv2d(128,512,kernel_size=6,padding=1,stride=4),
  nn.BatchNorm2d(512)
  ))
ops.append(nn.Sequential(
  nn.ReLU(),
...
def main():
  ...
  normal = torch.tensor([[0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 1.0000],
  ...
  reduce = torch.tensor([[0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000],
        [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000],
        [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000],
        [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000],
        [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000],
  ...
  model.set_arch_params([normal, reduce])
  ...
  for epoch in range(args.epochs):
    ...
    train_acc, train_obj = train(train_queue, model, criterion_1, optimizer, midden_net, ops)
...
def search(former_model, train_queue, valid_queue, model, architect, optimizer, lr, args):
  ...
  return differ_s_t, differ_GT

def train(train_queue, model, criterion, optimizer, midden_net, ops):
  ...
  for step, (input, target) in enumerate(train_queue):
    ...
    loss_l, loss_fea, loss_s_t = self_distillation(model, model.midden_output, target, midden_net, criterion, ops, args)
